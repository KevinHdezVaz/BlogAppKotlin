...
import com.kevin.courseApp.ui.openia.permission.MicrophonePermisoActivity
import com.kevin.courseApp.utils.openai.Hash
import com.kevin.courseApp.utils.openai.LocaleParser
...
class ChatActivity : FragmentActivity() {
    ...
    private suspend fun processRecording() {
        try {
            ...
            Toast.makeText(this, "Falla al grabar el audio", Toast.LENGTH_SHORT).show()
        ...
        if (isRecording) {
            ...
            if (ContextCompat.checkSelfPermission(
                ...
                permissionResultLauncherV2.launch(
                    Intent(
                        this,
                        MicrophonePermisoActivity::class.java
                    )
                )
            }
        }
    ...
    * */
    ...
    private fun writeImageToCache(bytes: ByteArray) {
        try {
            contentResolver.openFileDescriptor(
                Uri.fromFile(
                    File(getExternalFilesDir("images")?.absolutePath + "/" + Hash.hash(
                        Base64.getEncoder().encodeToString(bytes)) + ".png")
                ), "w")?.use {
                FileOutputStream(it.fileDescriptor).use {
                    it.write(
                        bytes
    ...
    private suspend fun generateImage(p: String) {
        try {
            ...
            val bytes: ByteArray =  IOUtils.toByteArray(`is`)
